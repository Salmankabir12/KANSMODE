...
const defaultColors = ['#0f172a', '#e94560', '#ff2a55'];
...
export function LiquidEther({
  ...
  useEffect(() => {
    ...
    const backgroundThreeColor = new THREE.Color(backgroundColor);
    const bgVec4 = new THREE.Vector4(
      backgroundThreeColor.r,
      backgroundThreeColor.g,
      backgroundThreeColor.b,
      1.0
    );
    ...
    class Simulation {
      ...
      getFloatType() { return /(iPad|iPhone|iPod|Android)/i.test(navigator.userAgent) ? THREE.HalfFloatType : THREE.FloatType; }
      createAllFBO() {
      ...
      calcSize() {
        const isLowEnd = typeof navigator !== 'undefined' && ((navigator.hardwareConcurrency && navigator.hardwareConcurrency <= 4) || ('deviceMemory' in navigator && (navigator as any).deviceMemory <= 4) || /(iPad|iPhone|iPod|Android)/i.test(navigator.userAgent));
        const effectiveResolution = isLowEnd ? Math.min(this.options.resolution, 0.35) : this.options.resolution;
        const w = Math.max(1, Math.round(effectiveResolution * Common.width));
        const h = Math.max(1, Math.round(effectiveResolution * Common.height));
  ...
  }, [colors, backgroundColor, autoDemo, autoSpeed, autoIntensity, takeoverDuration, autoResumeDelay, autoRampDuration]);
